...
function sameOrigin(request, env) {
  if (request.headers.get('Origin') !== origin(env)) throw new HttpError(403, 'Invalid request origin. Reload this page and try again.');
}
...
export async function kioskPage(request, env) {
  ...
  return page('Scan a key fob', `<p>Use the fob reader, then scan the QR code with your phone and sign in through Discord.</p>
    <form id="scanner-form"><label for="scanner">Fob scanner</label><input id="scanner" type="text" inputmode="none" autocomplete="off" spellcheck="false" maxlength="20" autofocus><button class="btn btn-outline" type="submit">Read fob</button></form>
    <p id="status" role="status" aria-live="polite">Ready to scan.</p><section id="claim" hidden><h2>Scan with your phone</h2><img id="qr" alt="Fob enrollment QR code"><p id="expiry"></p><p><a id="claim-link">Open enrollment link</a></p><button id="done" type="button" class="btn btn-outline">Done</button></section>
    <noscript><p>Enable JavaScript to use the fob reader and display QR codes.</p></noscript><p><a href="/waiver">Sign a waiver</a></p>`, true);
}
...
export async function kioskClaims(request, env) {
  ...
  if (request.method === 'GET') {
    ...
    if (params.getAll('token').length !== 1) throw new HttpError(400, 'An enrollment token is required.');
  ...
  if (request.headers.get('Content-Type')?.split(';')[0] !== 'application/json') throw new HttpError(415, 'Send a JSON fob scan.');
  let input;
  try { input = JSON.parse(await boundedText(request, 1024)); }
  catch (error) { if (error instanceof HttpError) throw error; throw new HttpError(400, 'Invalid fob scan.'); }
  const fob = input?.fob_id;
  if (!Number.isInteger(fob) || fob < 1 || fob > 4294967295) throw new HttpError(400, 'Enter a fob ID from 1 through 4294967295.');
