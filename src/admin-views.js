...
export function memberList(members, { total, current, pages, query = '' }, env, csrf) {
	const rows = members.map(m => `<tr><th scope="row"><a class="admin-member-name" href="${e(memberPath(m))}">${e(memberName(m))}</a><small>${e(m.discord_email || m.email || 'No email')}</small><small class="admin-id">Discord ID: ${e(m.discord_user_id || 'Not linked')}</small><small>${m.waiver_signed ? 'Waiver signed' : 'No linked waiver'}</small>${m.non_billable ? '<small>Non-billable</small>' : ''}${m.legacy_billing ? '<small>Legacy billing</small>' : ''}</th><td>${statusBadge(m)}<small>${timestamp(m.stripe_synced_at, true)}</small><small>${subscriptionLink(m, env)}</small></td><td>${m.bill_annually ? 'Yearly' : 'Monthly'}<small>${e(labelDiscount(m.discount_type))}</small></td></tr>`).join('');
