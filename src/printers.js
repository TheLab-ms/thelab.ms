...
export function printerOrigin(env) {
  ...
  throw new HttpError(503, 'Printer access is not configured. Set PRINTER_EDGE_URL to the HTTPS edge origin.');
}
...
export async function printerAccess(request, env) {
  ...
  if (!url.search) return redirect(`${edge}/printers/login`);
  if (!/^[a-f0-9]{64}$/.test(state || '') || url.searchParams.size !== 1) throw new HttpError(400, 'Invalid printer sign-in. Please start again from the printer page.');
  const member = await signedInMember(request, env);
  if (!member) return startLogin(request, env, 'member');
  if (!['active', 'trialing'].includes(member.stripe_subscription_state)) throw new HttpError(403, 'An active membership is required to view the 3D printers.');
  let key;
  try {
    ...
    throw new HttpError(503, 'Printer access signing is not configured correctly.');
  }
  const issued = now();
  ...
  return redirect(`${edge}/printers/callback#token=${data}.${encode(new Uint8Array(signature))}`);
}
