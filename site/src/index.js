...
export function memberList(members, { total, current, pages, query = '', filters = defaultMemberFilters }, env, csrf) {
	...
	const empty = query ? 'No members match your search.' : filtered ? 'No members match your search and filters.' : 'No members have registered yet.';
    ...
    <fieldset class="admin-list-filters" aria-label="Member filters"${query ? ' disabled' : ''}>${memberFilters.map(({ name, label, choices }) => select(name, label, filters[name], choices)).join('')}</fieldset>
    <div class="admin-actions"><button class="btn btn-primary" type="submit">${query ? 'Search' : 'Apply filters'}</button><a href="/admin">Reset</a></div></form><script src="/script.js" defer></script>
    <div class="admin-list-summary"><p><strong>${total} ${filtered ? 'matching' : 'registered'} member${total === 1 ? '' : 's'}</strong>${query ? ` for “${e(query)}”` : ''}</p><p class="admin-help">${query ? 'Searching all members · ' : filters.payment === 'all' ? 'Any payment status · ' : ''}Newest first</p></div><div class="admin-table" role="region" aria-label="Registered members" tabindex="0"><table class="admin-member-table"><thead><tr><th scope="col">Member</th><th scope="col">Subscription</th></tr></thead><tbody>${rows || `<tr><td colspan="2" class="admin-empty">${empty}${query ? '<p>Try changing your search.</p>' : filtered ? '<p>Try changing your search or filters.</p>' : ''}</td></tr>`}</tbody></table></div><nav class="admin-pagination" aria-label="Member pages">${current > 1 ? `<a class="btn btn-outline" href="${e(memberListURL(current - 1, query, filters))}">Previous</a>` : ''}<span>Page ${current} of ${pages}</span>${current < pages ? `<a class="btn btn-outline" href="${e(memberListURL(current + 1, query, filters))}">Next</a>` : ''}</nav>`, csrf, 200, env);
}
...
export function editor(member, fields, csrf, env, message = '', status = 200, events = [], waivers = '', checkoutURL = '') {
    ...
    <div class="admin-field" aria-label="Saved fob status"><span class="admin-status admin-status--${member.fob_enabled ? 'active' : 'inactive'}">Fob ${member.fob_enabled ? 'enabled' : 'disabled'}</span></div>
    ${input('fob_id', 'Fob ID', f.fob_id, 10, '')}
    ${checkbox('non_billable', 'Non-billable', f.non_billable, '')}
    ${checkbox('legacy_billing', 'Legacy billing', f.legacy_billing, '')}
    ...
    <aside class="admin-section admin-account" aria-labelledby="account-details"><h2 id="account-details">Account details</h2><dl class="admin-account-fields">${account.map(([label, value]) => `<div><dt>${e(label)}</dt><dd>${e(value || '—')}</dd></div>`).join('')}</dl><dl class="admin-account-dates">${dates.map(([label, value]) => `<div><dt>${e(label)}</dt><dd>${timestamp(value)}</dd></div>`).join('')}</dl></aside></div>
...
async function list(request, env, csrf) {
	...
	if (!query && filters.waiver !== 'all') conditions.push(`${waiverSignedSQL} = ${filters.waiver === 'signed' ? 1 : 0}`);
	if (!query && filters.discord !== 'all') conditions.push(`discord_user_id IS ${filters.discord === 'linked' ? 'NOT ' : ''}NULL`);
	if (!query && filters.payment !== 'all') {
...
export async function bindFob(request, env) {
	...
	return fobPage('Fob linked', `<p role="status">Fob ${claim.fob_id} is now linked to your membership.</p><p>Door access follows your membership and waiver status.</p><p>You can close this page.</p>`);
}
